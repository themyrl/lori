...
from .ops.modules import LoriMSDeformAttn
from .position_encoding import build_position_encoding

from lori.network_architecture.LORI_SwinUNet import ClassicAttention, Mlp
from timm.models.layers import DropPath

class LoriDeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        # encoder_layer = LoriDeformableTransformerEncoderLayer(d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, enc_n_points)

        self.encoder = LoriDeformableTransformerEncoder(num_encoder_layers, d_model, dim_feedforward, 
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, LoriMSDeformAttn):
                m._reset_parameters()
...
class LoriDeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # self attention
        self.self_attn = LoriMSDeformAttn(d_model, n_levels, n_heads, n_points, n_vt)
...
class LoriDeformableTransformerEncoder(nn.Module):
    def __init__(self, num_layers, d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, enc_n_points, n_levels, n_vt, vt_map):
        ...
        for i in range(num_layers):
            self.layers.append(
                LoriDeformableTransformerEncoderLayer(d_model, dim_feedforward, 
                                                    dropout, activation, 
